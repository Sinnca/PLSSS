...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['update_meeting_link'])) {
        $appointment_id = intval($_POST['appointment_id']);
        $meeting_link = filter_var($_POST['meeting_link'], FILTER_SANITIZE_URL);
        
        // Format the meeting link
        $formatted_link = trim($meeting_link);
        
        // If it's a Google Meet code (like xjb-gwtx-obn), format it as a full URL
        if (preg_match('/^[a-z0-9-]+$/i', $formatted_link)) {
            $formatted_link = 'https://meet.google.com/' . $formatted_link;
        }
        // If it's a meet.google.com link without protocol, add https://
        elseif (preg_match('/^meet\.google\.com\/[a-z0-9-]+$/i', $formatted_link)) {
            $formatted_link = 'https://' . $formatted_link;
        }
        // If it's a full URL, keep it as is
        elseif (!preg_match('~^(?:f|ht)tps?://~i', $formatted_link)) {
            $formatted_link = 'https://' . $formatted_link;
        }
        
        // Store the formatted link
        $sql = "UPDATE appointments SET meeting_link = ? WHERE id = ? AND consultant_id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sii", $formatted_link, $appointment_id, $consultant_id);
        
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION['success'] = "Meeting link updated successfully!";
        } else {
            $_SESSION['error'] = "Error updating meeting link: " . mysqli_error($conn);
        }
    }
...
<head>
    ...
    <style>
        ...
        .meeting-link-form input[type="url"] {
            border-radius: 20px;
            padding: 8px 15px;
...
<body>
    ...
    <div class="messages-container">
        ...
        <div class="meeting-link-form mb-4">
            <h5><i class="fas fa-video me-2"></i> Meeting Room</h5>
            <form method="POST" action="" class="mt-3">
                ...
                <div class="input-group">
                    <input type="text" 
                           name="meeting_link" 
                           class="form-control" 
                           placeholder="meet.google.com/your-meeting-id or just the code" 
                           value="<?php 
                               $link = !empty($appointment['meeting_link']) ? $appointment['meeting_link'] : '';
                               // If it's a full URL, show just the path part
                               if (preg_match('~^https?://(?:meet\.google\.com/)?([a-z0-9-]+)~i', $link, $matches)) {
                           ...
                           ?>"
                           required>
                    <button type="submit" name="update_meeting_link" class="btn btn-primary">
                        <i class="fas fa-save me-1"></i> Save Link
                    </button>
                </div>
                <small class="form-text text-muted">Enter a Google Meet code (e.g., xjb-gwtx-obn) or full URL</small>
            </form>
            <?php if (!empty($appointment['meeting_link'])): ?>
                <div class="mt-3">
                    <a href="<?php echo htmlspecialchars($appointment['meeting_link']); ?>" 
                    ...
                    <small class="text-muted ms-2">Share this link with the client</small>
                </div>
            <?php endif; ?>
        </div>
