...
<?php foreach($filtered_upcoming_appointments as $appt): ?>
    ...
    function loadAppointments() {
        ...
        const date = urlParams.get('date') || '<?php echo $today; ?>';
        const view = '<?php echo $view; ?>';
        
        console.log('Loading appointments for date:', date, 'view:', view);
        ...
        fetch(`../actions/fetch_appointments.php?date=${date}&view=${view}&consultant_id=<?php echo $_SESSION['consultant_id'] ?? 0; ?>`)
            .then(response => {
                if (!response.ok) {
    ...
    function renderDayView(appointments, date) {
        ...
        hours.forEach(({ hour24, display, hour12 }) => {
            // Find appointments for this hour
            const appts = appointments.filter(a => {
                try {
                    // Parse the appointment date
                    const apptDate = new Date(a.slot_date);
                    const apptDateStr = [
                    ...
                    const [apptHours] = a.slot_time.split(':').map(Number);
                    
                    // Check if appointment is on the current date and hour
                    const isSameDate = apptDateStr === currentDateStr;
                    const isSameHour = apptHours === hour24;
                    ...
                    console.log(`Checking appointment:`, {
                        ...
                        isSameDate,
                        isSameHour,
                        matches: isSameDate && isSameHour
                    });
                    
                    return isSameDate && isSameHour;
                } catch (e) {
                    console.error('Error processing appointment:', a, e);
                    return false;
                }
            });
            
            console.log(`Hour ${hour24}:00 (${display}): Found ${appts.length} appointments`, appts);
            
            html += `
                <tr>
                    <td class="time-cell">${display}</td>
        ...
        console.log('Rendered day view HTML:', html);
    ...
    function cancelAppointment(appointmentId) {
        ...
        // Disable the button
        const cancelBtn = document.querySelector(`.cancel-btn[data-id="${appointmentId}"]`);
        if (cancelBtn) {
    ...
    // Single event listener for cancel buttons using event delegation
    document.addEventListener('click', function(e) {
        // Only handle clicks directly on cancel buttons or their icons
